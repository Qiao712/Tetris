...
#ifdef __linux__
	#define LIST_BLOCKS  "res/list_blocks"
	#define LIST_STICKERS "res/list_stickers"
#elif _WIN32	
	#define LIST_BLOCKS  "res\\list_blocks"
	#define LIST_STICKERS "res\\list_stickers"
#endif
...
BlockFactory::BlockFactory()
{
	...
	//读取记录方块资源列表文件
	char file_name[1024];
	ifstream file_list_block(LIST_BLOCKS);
	...
	//读取记录贴纸资源列表文件----------------
	ifstream file_list_stickers(LIST_STICKERS);
	if (!file_list_stickers.good()) {
		//cout << "BlockFactory::BlockFactory: res/list_stickers can't be open." << endl;
		return;
	}
	while (!file_list_stickers.eof()) {
		file_list_stickers >> file_name;
